...
const searchFilm = async(find_object)=> {
    try {
        const film = await modelFilm.find(find_object);
        return film;
    } catch (error) {
        return error;
    }
...
const deleteFilm = async(id_film)=> {
    try {
        const film = await modelFilm.deleteOne({_id: id_film});
        return film;
    } catch (error) {
        return error;
    }
...
app.get("/film/title", function(req, res) {
    ...
    if (titleFilm == undefined) {
        res.status(400).send("Parametro titolo non trovato.");
    }

    const find_object = {title: titleFilm};
...
app.post("/film", function(req, res){
    if(req.body.title == undefined || req.body.title.length==0) {
        res.status(400).send("titolo film mancante");
    }
...
app.delete("/film", function(req, res){
    ...
    const id_da_cancellare = req.body.id;
    const film = deleteFilm(id_da_cancellare);
    film.then((item)=> {
        res.send(item);
    })
